...
class AES:
    ...
    # Convert master key into a matrix and initialize round keys
        ...
        # Generate all 44 round keys (11 rounds × 4 words per round)
        for i in range(4, 4 * 11):
            self.round_keys.append([])
            if i % 4 == 0:  # Apply the key schedule core
                t = self.round_keys[i - 1][1:] + self.round_keys[i - 1][:1]  # Rotate word
                t = [Sbox[byte] for byte in t]  # Apply Sbox substitution
                t[0] ^= Rcon[i // 4]  # XOR with round constant
                # XOR with the corresponding word 4 positions earlier
                for j in range(4):
                    self.round_keys[i].append(self.round_keys[i - 4][j] ^ t[j])
            else:  # No special transformation
                for j in range(4):
                    self.round_keys[i].append(self.round_keys[i - 4][j] ^ self.round_keys[i - 1][j])
